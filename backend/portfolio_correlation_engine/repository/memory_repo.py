...
from backend.portfolio_correlation_engine.exceptions import PortfolioCorrelationRepositoryError

class MemoryPortfolioCorrelationRepository(IPortfolioCorrelationRepository):
    """
    In-memory INSERT-only implementation of the Portfolio Correlation Repository.
    """
    def __init__(self):
        self._store: Dict[str, PortfolioCorrelationSnapshot] = {}
        self._lock = asyncio.Lock()
        
    async def save(self, snapshot: PortfolioCorrelationSnapshot) -> None:
        async with self._lock:
            if snapshot.snapshot_id in self._store:
                raise PortfolioCorrelationRepositoryError(f"Snapshot {snapshot.snapshot_id} already exists. Updates are strictly forbidden.")
